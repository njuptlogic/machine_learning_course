...
lr = 0.0004  # The optimization initial learning rate
...
def load_data(mode='train'):
    """
    Function to (download and) load the MNIST data using tensorflow_datasets
    :param mode: train or test
    :return: images and the corresponding labels
    """
...
def preprocess_data(dataset):
    """
    Preprocess the MNIST dataset to match the input format for the model.
    :param dataset: TensorFlow dataset (either train or test)
    ...
    def _preprocess_image(image, label):
        ...
        image = tf.reshape(image, [img_h, img_w, n_channels])  # Reshape image
        label = tf.one_hot(label, n_classes)  # One-hot encoding for labels
        return image, label

    dataset = dataset.map(_preprocess_image)
    dataset = dataset.batch(100)  # Set batch size
    return dataset


# Network functions (same as before)
def weight_variable(shape):
    initer = tf.truncated_normal_initializer(stddev=0.01)
...
conv1 = conv_layer(x, filter_size1=5, num_filters=16, stride=1, name='conv1')
pool1 = max_pool(conv1, ksize=2, stride=2, name='pool1')
conv2 = conv_layer(pool1, filter_size2=5, num_filters=32, stride=1, name='conv2')
pool2 = max_pool(conv2, ksize=2, stride=2, name='pool2')
layer_flat = flatten_layer(pool2)
fc1 = fc_layer(layer_flat, num_units=128, name='FC1', use_relu=True)
output_logits = fc_layer(fc1, n_classes, name='OUT', use_relu=False)
...
for epoch in range(10):
    ...
    val_data = load_data(mode='test')
    val_loss, val_acc = sess.run([loss, accuracy], feed_dict=feed_dict_valid)
    print(f"Epoch {epoch + 1}, Validation loss: {val_loss:.2f}, Validation accuracy: {val_acc:.01%}")

# Test the network after training
test_data = load_data(mode='test')
loss_test, acc_test = sess.run([loss, accuracy], feed_dict=test_data)
print(f"Test loss: {loss_test:.2f}, Test accuracy: {acc_test:.01%}")
'''
